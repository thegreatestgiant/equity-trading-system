#!/usr/bin/env bash
# remote_worker_up.sh - Enhanced for cross-environment portability
# Usage: sudo K3S_TOKEN=<token> CONTROL_PLANE_IP=<ip> IFACE=<iface> ./remote_worker_up.sh
...
# Set default interface to eth0 if not provided
INTERFACE=${IFACE:-eth0}

echo "🚀 Bootstrapping K3s Worker Node on interface: ${INTERFACE}..."
